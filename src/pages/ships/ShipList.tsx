...
import { listShipAPI, createShipAPI, updateShipAPI, shipDetailAPI } from '../../features/API/ship/Ship.ts';
...
import { Plus, Pencil, Trash2, AlertCircle, Eye, Search, ChevronLeft, ChevronRight, X, Ship as ShipIcon } from 'lucide-react';
                ...
                setShips(currentShips =>
                    currentShips.map(s => {
                        ...
                        if (match && match.status === 'fulfilled' && match.value) {
                            const val = match.value as { id: string; serial?: string; deviceSerial?: string; ownerName?: string; idshipOwner?: string };
                            ...
                            return {
                                ...
                                ownerName: owner || (s as any).ownerName
                            };
                        }
                        return s;
                    })
            ...
            <div className="table-container flex-1 flex flex-col">
                ...
                <div className="flex items-center justify-between" style={{ padding: '12px 20px', borderTop: '1px solid var(--border-color)', backgroundColor: '#ffffff' }}>
                    <div className="text-sm" style={{ color: '#64748b' }}>
                        Hiển thị <strong>{ships.length > 0 ? (page - 1) * 10 + 1 : 0}</strong> - <strong>{Math.min(page * 10, totalCount)}</strong> trong tổng số <strong>{totalCount}</strong> tàu
                    </div>
                    {totalPages > 1 && (
                        <div className="flex items-center gap-sm">
                            <button 
