...
export default function OwnerList() {
    ...
    const fetchOwners = useCallback(async (keyword: string, currentPage: number) => {
        try {
            ...
            const initialized = rawList.map(o => ({
                ...o,
                phone: o.phone || (o as ShipOwner & { Phone?: string, Address?: string }).Phone || undefined,
                address: o.address || (o as ShipOwner & { Phone?: string, Address?: string }).Address || undefined,
            }));
            ...
            setError((err as Error).message || 'Không thể tải danh sách chủ tàu');
        } finally {
    ...
    useEffect(() => {
        setTimeout(() => void fetchOwners(search, page), 0);
    }, [page, search, fetchOwners]); // Chạy lại khi chuyển trang
    ...
    const handleModalSubmit = async (payload: ShipOwnerPayload) => {
        ...
        void fetchOwners(search, page); // Tải lại trang hiện tại
    };
                    ...
                    <table className="table">
                        ...
                        {isLoading ? (
                            ...
                            owners.map((owner, index) => {
                                const phoneVal = owner.phone || (owner as ShipOwner & {
                                    Phone?: string,
                                    Address?: string
                                }).Phone;
                                const addressVal = owner.address || (owner as ShipOwner & {
                                    Phone?: string,
                                    Address?: string
                                }).Address;
                                return (
                                    <tr key={owner.id}>
                                        ...
                                        <td>
                                                <span className="material-chip chip-gray" style={{
                                                    fontWeight: 600,
                                                    fontSize: '13px',
                                        ...
                                        <td>
                                            {phoneVal ? (
                                                ...
                                                <span className="text-muted"
                                                      style={{fontStyle: 'italic', fontSize: '13px'}}>-</span>
                                            )}
                                        </td>
                                        <td>
                                            {addressVal ? (
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center gap-sm">
                            ...
                            <span className="text-sm font-semibold px-2"
                                  style={{color: '#334155'}}>Trang {page} / {totalPages}</span>
                            <button
                                className="btn btn-outline"
                                style={{padding: '6px 12px', borderRadius: '8px', fontSize: '13px'}}
